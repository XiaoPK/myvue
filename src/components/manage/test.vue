<template>
  <div>
    ...
    <el-row class="right">
      ...
      <el-button size="medium" type="primary" @click="dialogUploadVisible = true">导入数据</el-button>
    ...
    >
        <el-table-column type="selection" width="50" slot="reference"></el-table-column>
    ...
    <el-dialog title="导入数据" :visible.sync="dialogUploadVisible" width="400px">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="importCache()">完 成</el-button>
      </div>
    </el-dialog>
    <!-- 上传数据结束 -->
  </div>
    ...
    getErrors(param){
      for (let i = 0; i < param.length; i++) {
        if(param[i].isError != ''){
          ...
          this.errorTipCache += "教师编号"+param[i].teacherNumber + '发生错误：' + param[i].isError + "'\'n"
        }
      }
      console.log(this.errorTipCache)
      this.errorTip = this.errorTipCache
      return 
    },
    handleSuccess(res) {
      console.log("文件上传成功");
